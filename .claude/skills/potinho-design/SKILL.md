...
### Cores dark mode (exclusivas do admin)

| Token Tailwind | Hex | Papel |
|---|---|---|
| `potinho-noite` | `#1E1712` | Fundo de página e superfícies recessadas (equivalente escuro de `potinho-fundo`) |
| `potinho-carvao` | `#2B2019` | Fundo de cards/painéis elevados (equivalente escuro de `bg-white`) |
| `potinho-caramelo` | `#D8A878` | Títulos, preços e destaques de texto (equivalente escuro de `text-potinho-chocolate` **só como tinta**, nunca como fundo de botão) |

Só existem para escopar dark mode à área `/admin` — nunca usar na loja. Ver seção 11.
...
## 10. Dark mode (só admin)

A loja (páginas de marca, checkout, `/conta`) **não tem dark mode** — é sempre clara, é parte da identidade cinematográfica. Só a área `/admin` (ferramenta interna, não brand-facing) tem dark mode, com escopo isolado:

- **Ativação:** classe `.dark` num único wrapper, `<div id="admin-root">` em `web/src/app/admin/layout.tsx`. O variant `dark:` é redefinido em `globals.css` como `@custom-variant dark (&:where(.dark, .dark *));` (classe, não `prefers-color-scheme`) — por isso nunca vaza pra loja, que nunca recebe essa classe.
- **Toggle:** `DarkModeToggle` (`web/src/components/admin/DarkModeToggle.tsx`), persiste em `localStorage["potinho-admin-theme"]`. Um script inline no layout aplica a classe antes do 1º paint (evita flash), com fallback pra `prefers-color-scheme` quando não há preferência salva.
- **Regra mecânica de conversão** (mesma relação de profundidade clara↔escura, sempre nessa direção):
  - `bg-potinho-fundo` (superfície recessada: página, inputs, painel aninhado dentro de card) → soma `dark:bg-potinho-noite`
  - `bg-white` (superfície elevada: card, input que precisa "saltar" sobre um `fundo`) → soma `dark:bg-potinho-carvao`
  - `text-potinho-chocolate` (títulos, preços, destaque) → soma `dark:text-potinho-caramelo`
  - `text-potinho-texto`(`/NN`) → soma `dark:text-potinho-bege`(`/NN` igual)
  - `border-potinho-bege` / `divide-potinho-bege` → soma `dark:border-potinho-cinza/30` (ou `dark:divide-potinho-cinza/20`)
  - `hover:bg-potinho-fundo` / `hover:bg-white` (hover neutro) → soma `dark:hover:bg-white/5` (nunca reusar `carvao`/`noite` num hover, eles são fundo de superfície, não tinta de hover)
  - Badges de status com cor fora da marca (`sky-100`/`emerald-100`/`zinc-200` etc.) → par escuro (`dark:bg-sky-950 dark:text-sky-300`, etc.)
- **Nunca remapear `potinho-chocolate` nem `potinho-bege` como variável CSS global.** Eles são usados tanto como tinta quanto como fundo de botão sólido (`bg-potinho-chocolate text-potinho-bege`) — remapear a variável quebraria o contraste do par botão. Por isso o dark mode é aditivo (`dark:` por classe), nunca um override de `@theme`. Botões/badges de fundo chocolate ficam **idênticos** em ambos os temas — já têm contraste alto e "flutuam" bem sobre fundo escuro.
- Copiar o padrão de um arquivo já convertido (`web/src/app/admin/page.tsx` ou `web/src/components/admin/ProdutoForm.tsx`) em vez de reinventar a combinação.
